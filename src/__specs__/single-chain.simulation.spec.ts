...
import { bnbBNB, bnbDAI, bnbUSDC, bnbUSDT, bnbWBNB, sdkConfig } from "~/__specs__/utils/sdk-test-config"
...
describe("Single chain simulation", async () => {
  ...
  it("should be able to execute one-to-one transactions", async () => {
    const executeTx = await createRequest([bnbUSDT], [100])
  ...
  it("should be able to execute one-to-many transactions", async () => {
    const executeTx = await createRequest([bnbUSDT, bnbUSDC, bnbWBNB, bnbDAI], [25, 25, 25, 25])
